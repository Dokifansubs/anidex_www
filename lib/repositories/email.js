...
EM.dispatchRecoverPassword = function (account, callback) {
    EM.server.send({
        from: ES.sender,
        to: account.email,
        subject: 'AniDex Password Recovery',
        text: 'AnidDex password recovery.',
        attachment: EM.composeRecoverLink(account)
    }, function (err, message) {
        callback(err, message);
    });
};

EM.composeRecoverLink = function (o) {
    var link = 'http://' + ServerConfig.domain + '/recover-password?h=' + o.ident_hash;
    var html = '<html><body>';
        html += 'Hi ' + o.username + ',<br><br>';
        html += 'This email contains a link to recover your account<br><br>';
        html += '<a href="' + link + '">' + link + '</a><br><br>';
        html += 'Greetings,<br>';
        html += 'The AniDex Team';
        html += '</body></html>';
