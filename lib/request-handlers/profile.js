...
exports.register = function(req, res) {
    ...
    res.render('register', {title: 'AniDex', user: req.user, message: req.session.message, error: req.session.error, public_key: CaptchaConf.public_key});
...
exports.verify = function(req, res) {
    ...
    database.verifyUser(hash, function(err, status) {
        if (err) {
            return res.render('verify-account', { title: 'AniDex', error: 'Database error.'});
        }
        switch (status) {
            case 2: return res.render('verify-account', { title: 'AniDex', error: 'Account not found' }); break;
            case 0: return res.render('verify-account', { title: 'AniDex', message: 'Account verified.' }); break;
            default: return res.render('verify-account', { title: 'AniDex', error: 'Unknown error.'}); break;
        }
    })
}
