...
exports.deleteFile = function(req, res) {
    var torrents = req.body.torrent;
    if (!Array.isArray(torrents)) {
        torrents = [torrents];
    }

    if (!req.isAuthenticated()) {
        return res.send(401, {error: 'user not authenticated'})
    }

    database.deleteTorrent(torrents, req.user, function(err, result) {
        if (err) {
            console.log(err);
            req.flash('error', 'Database error.');
        } else {
            _.forEach(result.info_hash, function(item) {
                fs.unlink('./public/torrents/' + item + '.btf', function() {});
            });
            req.flash('info', 'Deleted ' + result.affectedRows + ' torrent' + (result.affectedRows == 1 ? '' : 's') + '.');
        }
        res.redirect('/account/torrents/');
    })
}
