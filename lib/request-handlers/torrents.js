...
exports.upload = function(req, res) {
    database.getCategories(function(err, data) {
        res.render('upload', {title: 'AniDex', user: req.user, message: req.session.message, categories: data});
...
exports.downloadFile = function(req, res) {
    database.getTorrent(req.param('id'), function(err, data) {
        if (!data) { return res.render('/', {title: 'AniDex', user: req.user}); }
...
exports.uploadFile = function(req, res) {
    ...
    torrents.forEach(function(item) {
        if (item.type === 'application/x-bittorrent') {
            ...
            parsed.filename = req.body.filename;
            ...
            database.addTorrent(parsed, function(err, torrentid) {
                ...
                fs.rename(item.path, './public/torrents/' + parsed.infoHash + '.btf', function(err) {
                    if (err) {
                        ...
                        req.session.message = {
                            type: 1,
                            message: 'Unable to write torrent to disk, please inform an administrator.'
                        }
                        return res.redirect('/upload');
                    }
                    req.session.message = {
                        ...
                        link: 'http://anidex.moe:8008/torrents/' + torrentid
                    }
                    return res.redirect('/upload');
